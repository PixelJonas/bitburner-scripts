import { NS } from "@ns";
import { getContracts } from "./lib/infoCache";
...
export async function main(ns: NS): Promise<void> {
  ...
  let contractOrders = getContracts(ns);

  if (!contractOrders) {
    ns.tprint(`FAIL: Could not load any contracts. Did you run prepContracts.js before this script?`);
    return;
  }

  let found = 0;
  for (let contract of contractOrders) {
    found++;
    let solved = "";

    if (!silent) {
      ns.tprint(`------ NEW CONTRACT -----`);
    ...
    try { solved = await solve(contract, ns) } catch { }
    if (solved) {
      ns.tprint(`INFO: Solved ${contract.type}`)
    } else {
      ns.tprint(`FAIL: ${contract.server} - ${contract.file} - ${contract.type} ${solved || "FAILED!"}`);
    }
...
async function solve(contract: ContractOrder, ns: NS) {
  let solution: any = "none";

  switch (contract.type) {
    case "Algorithmic Stock Trader I":
      solution = maxProfit([1, contract.data]);
      break;
    case "Algorithmic Stock Trader II":
      solution = maxProfit([Math.ceil(contract.data.length / 2), contract.data]);
      break;
    case "Algorithmic Stock Trader III":
      solution = maxProfit([2, contract.data]);
      break;
    case "Algorithmic Stock Trader IV":
      solution = maxProfit(contract.data);
      break;
    case "Minimum Path Sum in a Triangle":
      solution = solveTriangleSum(contract.data, ns);
      break;
    case "Unique Paths in a Grid I":
      solution = uniquePathsI(contract.data);
      break;
    case "Unique Paths in a Grid II":
      solution = uniquePathsII(contract.data);
      break;
    case "Generate IP Addresses":
      solution = generateIps(contract.data);
      break;
    case "Find Largest Prime Factor":
      solution = factor3(contract.data);
      break;
    case "Spiralize Matrix":
      solution = spiral(contract.data);
      break;
    case "Merge Overlapping Intervals":
      solution = mergeOverlap(contract.data);
      break;
    case "Find All Valid Math Expressions":
      solution = solverWaysToExpress(ns, contract.data);
      break;
    case "Subarray with Maximum Sum":
      solution = solverLargestSubset(ns, contract.data);
      break;
    case "Sanitize Parentheses in Expression":
      solution = SanitizeParensSolver(contract.data);
      break;
    case "Array Jumping Game":
      solution = solverArrayJumpingGame(contract.data)
      break;
    case "Total Ways to Sum":
      solution = solverWaysToSum(contract.data);
      break;
    case "Total Ways to Sum II":
      solution = solveWaysToSumII(contract.data);
      break;
    case "HammingCodes: Integer to Encoded Binary":
      solution = HammingEncode(contract.data);
      break;
    case "HammingCodes: Encoded Binary to Integer":
      solution = HammingDecode(contract.data);
      break;
    case "Shortest Path in a Grid":
      solution = ShortestPathInAGrid(contract.data);
      break;
    case "Array Jumping Game II":
      solution = solverArrayJumpingGameII(contract.data);
      break;
    case "Compression II: LZ Decompression":
      solution = comprLZDecode(contract.data);
      break;
    case "Proper 2-Coloring of a Graph":
      solution = coloringGraph(contract.data);
      break;
    case "Compression III: LZ Compression":
      solution = comprLZEncode(contract.data);
      break;
    case "Compression I: RLE Compression":
      solution = RLEencode(contract.data);
      break;
    case "Encryption I: Caesar Cipher":
      solution = caesarCipher(contract.data);
      break;
    case "Encryption II: Vigenère Cipher":
      solution = vignereCipher(contract.data);
      break;
  }

  if (solution == 'none') {
    ns.tprint(`ERROR: No Solver for ${contract.file}`);
    return "";
  }
  let reward = ns.codingcontract.attempt(solution, contract.file, contract.server);
  if (!reward) {
    ns.tprint(`-----------------------------------`);
    ns.tprint(`FAIL: Could not solve ${contract.file} on ${contract.server}. Type: ${contract.type}`);
    ns.tprint(`FAIL: Data: ${contract.data}`);
    ns.tprint(`FAIL: Solution: ${solution}`);
    ns.tprint(`-----------------------------------`);
  }
  return reward;
}

function vignereCipher(data: any) {
  const cipher = [...data[0]]
    .map((a, i) => {
      return a === " "
        ? a
...
function caesarCipher(data: any) {
  const cipher = [...data[0]]
...
function solverArrayJumpingGameII(arrayData: any) {
 ...
 * @returns
 */
function solveWaysToSumII(input: any[]) {
  /**
   *
   * @param {number} target
   * @param {number[]} nums
...
function maxProfit(arrayData: any[]) {
  let i, j, k;
...
function solveTriangleSum(arrayData: any, ns: NS) {
  ...
  return Math.min.apply(null, nextArray as any[]);
}

//UNIQUE PATHS IN A GRID
function uniquePathsI(grid: number[]) {
...
function factorialDivision(n: number, d: number): any {
  if (n == 0 || n == 1 || n == d)
    return 1;
  return factorialDivision(n - 1, d) * n;
}

function uniquePathsII(grid: string | any[], ignoreFirst = false, ignoreLast = false) {
...
function generateIps(num: string | any[]) {
  num = num.toString();
...
function isValidIpSegment(segment: string | number | string[]) {
  if ((segment as string[])[0] == "0" && segment != "0") return false;
...
function column(arr: any, index: any) {
  const res = [];
...
function mergeOverlap(intervals: any[][]) {
...
function SanitizeParensSolver(data: string | any[]) {
  var left = 0
  var right = 0
  var res: any[] = []
  ...
  function dfs(pair: number, index: number, left: number, right: number, s: string | any[], solution: string, res: any[]) {
    if (s.length === index) {
...
function solverArrayJumpingGame(arrayData: string | any[]) {
  ...
  return 0 + (Boolean(arrayJump[arrayData.length - 1]) as any); // thanks /u/Kalumniatoris
}

function solverWaysToSum(arrayData: number) {
  var ways = [];
  ways[0] = 1;
...
function solverWaysToExpress(ns: NS, arrayData: any[]): string {
  let i, j, k;
...
function solverLargestSubset(ns: NS, arrayData: string | any[]) {
...
export function HammingSumOfParity(_lengthOfDBits: number) { // will calculate the needed amount of parityBits 'without' the "overall"-Parity
...
export function HammingEncode(value: { toString: (arg0: number) => any; }) {
  ...
  let count = (arr: any[], val: string) => arr.reduce((a: number, v: any) => (v === val ? a + 1 : a), 0);
  ...
  for (let index of _build.reduce(function (a, e, i) { if (e == "x") a.push(i); return a; }, [])) {
    ...
    let _tempdata: any = [..._build]; // copy the "build"
...
export function HammingDecode(_data: string) {
  ...
  let count = (arr: any[], val: string) => arr.reduce((a: number, v: any) => (v === val ? a + 1 : a), 0); // count.... again ;)
...
function comprLZDecode(compr: string) {
  let plain = "";
...
function coloringGraph(data: any[]) {
...
export function comprLZEncode(plain: string) {
  // for state[i][j]:
...
function RLEencode(data: Iterable<unknown> | ArrayLike<unknown>) {
